...
import { generateCurl, generateJavaScript } from '@/lib/codeGeneration';
import { ResponsePanel } from './ResponsePanel';

function MethodBadge({ method, className }: { method: string; className?: string }) {
  const colors = METHOD_COLORS[method.toUpperCase()] || METHOD_COLOR_DEFAULT;
  return (
    <span
...
export function TryItModal({
  ...
  const {
    token, setToken,
    bodyFields, setBodyFields, updateBodyField,
    response, loading, error,
    hasBody, getBodyJSON, buildFullUrl, handleSend,
  } = useApiRequest(baseUrl, path, method);
  ...
  useEffect(() => {
    if (bodyParams.length > 0 && Object.keys(bodyFields).length === 0) {
      ...
      bodyParams.forEach(param => { initialFields[param.name] = ''; });
      setBodyFields(initialFields);
    }
  }, [bodyParams, bodyFields, setBodyFields]);
  ...
  const codeGenOpts = { method, fullUrl, token, hasBody, bodyJSON };
  const curlLines = generateCurl(codeGenOpts);
  const jsCode = generateJavaScript(codeGenOpts);
      ...
      >
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          <div className="w-[480px] flex-shrink-0 overflow-auto p-5 space-y-4 scrollbar-hide" style={{ backgroundColor: isDark ? '#09090b' : '#fafafa' }}>
            ...
            {/* Response */}
            {response && (
              <ResponsePanel response={response} isDark={isDark} monoFont={monoFont} />
            )}
          </div>
        </div>
      </div>
